class CardSwipe extends HTMLElement {
  ...
  actionizeCard(card){
    ...
    const maxT=card_wrap_height * .5
    const maxL=card_wrap_width * .5
    const abs=Math.abs
    let sX=false
    let sY=false
  ...
  generateNext(){
    ...
    node.className="node"
    node.style.cssText="height:90%;width:90%;background:#2F313D;border-radius:30px;color:white;"
    const _this=this
    let word
    ...
    <div class="frame" style="height:40 %;width:100%;background:;">
      <img style="height:70vw;width:70vw;border-radius:30px;" src="${word.image || 'https://cdn0.tnwcdn.com/wp-content/blogs.dir/1/files/2015/04/usertesting-730x356.jpg'}"></img>
    </div>
